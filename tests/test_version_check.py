...
class TestParseMajorVersion:
    ...
    def test_version_with_cuda_suffix(self):
    ...
    def test_version_with_rocm_suffix(self):
    ...
    def test_dev_version_with_git_hash(self):
    ...
    def test_dev_version_with_device_suffix(self):
...
class TestCheckVllmCompatibility:
    ...
    def test_compatible_versions(self, mock_logger, mock_version):
    ...
    def test_incompatible_versions_error_action(self, mock_exit, mock_logger, mock_version):
    ...
    @patch("vllm_omni.version.__version__", "0.15.0+cuda")
    @patch("vllm_omni.utils.version_check.logger")
    def test_incompatible_versions_warn_action(self, mock_logger, mock_version):
    ...
    @patch("vllm_omni.utils.version_check.logger")
    def test_skip_action(self, mock_logger):
        """Test that skip action bypasses the check."""
        # Note: "skip" action is no longer supported in the current implementation
        # The function only accepts "warn" or "error"
        # This test should be removed or updated based on requirements
        pass

    @patch("vllm_omni.utils.version_check.importlib.metadata.version")
    @patch("vllm_omni.utils.version_check.logger")
    @patch("vllm_omni.utils.version_check.sys.exit")
    def test_vllm_not_installed(self, mock_exit, mock_logger, mock_version):
    ...
    @patch("vllm_omni.utils.version_check.importlib.metadata.version")
    @patch("vllm_omni.utils.version_check.logger")
    def test_vllm_not_installed_warn_action(self, mock_logger, mock_version):
    ...
    @patch("vllm_omni.utils.version_check.logger")
    def test_version_parse_failure(self, mock_logger, mock_version):
    ...
    @patch("vllm_omni.utils.version_check.logger")
    def test_environment_variable_override(self, mock_logger, mock_version):
    ...
    @patch("vllm_omni.utils.version_check.logger")
    def test_dev_versions_compatibility(self, mock_logger, mock_version):
    ...
    @patch("vllm_omni.utils.version_check.importlib.metadata.version")
    @patch("vllm_omni.utils.version_check.logger")
    def test_unexpected_exception(self, mock_logger, mock_version):
